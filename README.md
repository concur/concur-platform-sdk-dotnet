...
To prevent time-out or latency when issuing web service calls, our ConcurPlatform library allows data pagination. One web service call is issued for each data page requested, and the call is only issued when the library consumer requests the next page. Pagination is supported on operations that return a collection of objects from Concur (i.e. GetExpenseReportsAsync, GetExpenseEntriesAsync, GetExpenseAttendeesAsync, etc.). Operations that support pagination always have an integer parameter named __limit__ for informing the page size, and a string parameter named __offset__ for informing where the next page begins. Also, the responded data page always contain a method named __IsEndOfPagination( )__ for informing whether the current is the last page, and another method named __NextPageOffset( )__ to inform the offset to be used when requesting the next page. The pagination sort order is based on an internal field managed by Concur and hidden from the public, but in general the pagination order for an object is the same as the object's creation date at Concur. 
